...
### Belt-and-suspenders (LANDED)

The mechanism+post-condition wording is now also in the integrator's `SOUL.md`
template (`src/scientia/references/soul-integrator.md.tmpl`, Merge Discipline §4),
not only the card body — so the system prompt itself enforces "change the
assignee (a board reassign event), a comment is not a reassignment, then re-read
and confirm" even if a card body is truncated.
...
| `src/scientia/references/soul-integrator.md.tmpl` | Merge Discipline §4 now carries the verified-reassign mechanism (change assignee + re-read to confirm) in the integrator's system prompt itself. |
