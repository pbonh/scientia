...
- **`scientia-hermes-status` detector:** flag any `blocked` card whose reason
  mentions reassignment but whose `assignee` is unchanged as a *real*
  escalation. This is the one check that would have caught the dead-end
  automatically.
