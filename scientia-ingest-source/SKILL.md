...
the deterministic `scientia` package owns every derived number. You never set
`effective` by hand — `scientia.confidence.recompute` does that.

## Inputs

- A single path to a raw file under `sources/` (resolve via
  `scientia.paths.sources_dir()`).
...
`article` | …) — `scientia-seed-proposal` and `scientia-grill-proposal` key off it. A **claim**
...
4. **For each candidate claim, check for a near-duplicate** among existing claim
   pages (`scientia.wiki.list_pages(wiki_dir, type="claim")`). If it closely
...
6. **Inline contradiction check** against immediate neighbors
   (`scientia.wiki.neighbors(page, wiki_dir, hops=1)`). On a real
...
Use `scientia.paths` for every path; never hard-code one.
...
- `pause_and_ask`: if `base < thresholds.low_confidence_floor` (0.45), emit
  ...
  `scientia.paths.question_for_operator_path` and halt.
