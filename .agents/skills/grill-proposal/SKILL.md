...
- The entire wiki, for query (`kg_pipeline.wiki`, `kg_pipeline.confidence`).
...
`kg_pipeline.validators.validate_grill` reports its entries and
`kg_pipeline.advance.advance(change_id, "grill")` refuses to stamp the marker.
