"""kg_pipeline.paths — the single source of file-layout truth (ADR-0005).
...
with the ``KG_PIPELINE_ROOT`` environment variable, which keeps the helpers pure
...
def project_root() -> Path:
    ...
    ``KG_PIPELINE_ROOT`` wins when set; otherwise the current working directory.
    """
    env = os.environ.get("KG_PIPELINE_ROOT")
    if env:
...
def references_dir() -> Path:
    ...
    shipped defaults); otherwise falls back to the bundle's own ``references/``
    that ships alongside this package.
    """
