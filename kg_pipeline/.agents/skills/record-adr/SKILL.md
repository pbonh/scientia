...
- `proposals/<change-id>/adrs/<NNNN>-<kebab-title>.md`, one per decision,
  rendered from the `adr` template via `kg_pipeline.templates`. ADR numbers are
...
`kg_pipeline.validators.validate_adrs` checks that every ADR has Status,
...
assertions and the wiki claims they cite (use `kg_pipeline.confidence.rollup_*`
